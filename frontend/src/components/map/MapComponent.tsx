...
const SIGNAL_FILTER_STYLES: Record<
  SignalRange,
  { active: string; inactive: string }
> = {
  All: {
    active: "bg-[#2B67EB] text-white border-[#2B67EB]",
    inactive: "bg-white/90 text-gray-700 border-gray-200 hover:border-gray-300",
  },
  Strong: {
...
const MapComponent: React.FC<Props> = ({
  ...
  const fetchLiveTowers = useCallback(
  ...
  const matchesSignalRange = (tower: Tower) => {
    ...
    if (signalRange === "Moderate")
      return tower.signal > 0.45 && tower.signal <= 0.7;
    return tower.signal <= 0.45;
  };

  // Prefer live API towers; fall back to empty array (no misleading mock pins)
  const baseTowers: Tower[] =
    liveTowers.length > 0
      ? liveTowers.map((t) => ({
          id: t.id,
          provider:
            (t.provider as Exclude<Provider, "All">) in PROVIDER_COLORS
  ...
  const filteredTowers = baseTowers.filter(
    (tower) =>
      (selectedProvider === "All" || tower.provider === selectedProvider) &&
            ...
            >
              ...
              {showLayers &&
                liveTowers
                  ...
                  .map((tower) => {
                    const providerKey = tower.provider as Exclude<
                      Provider,
                      "All"
                    >;
